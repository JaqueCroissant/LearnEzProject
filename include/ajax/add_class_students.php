...
if(isset($_POST))
{
    ...
    $students_to_remove = isset($_POST['students_to_remove']) ? $_POST['students_to_remove'] : array();
    
    $all_students = array_merge($students_to_add, $students_to_remove);
    $class_array = array();
    $class_array[] = $class_id;

    if($schoolHandler->school_has_classes($school_id, $class_array))
    {
       if($classHandler->remove_user_from_class($all_students, $class_id) && $classHandler->add_user_to_class($students_to_add, $class_id))
       {
