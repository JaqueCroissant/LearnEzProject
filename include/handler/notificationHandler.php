    ...
    class NotificationHandler
    {
        public function getNumberOfUnread($userId){
            if (is_numeric($userId)) {
                return DbHandler::get_instance()->CountQuery("SELECT notification_id FROM user_notifications WHERE user_id=:userId AND is_read=0", $userId);
            }
            return 0;
        }
        ...
        public function readNotification($notificationId){
            if (is_numeric($userId)) {
                DbHandler::get_instance()->Query("UPDATE user_notification SET is_read=1 WHERE id=:notificationId", $notificationId);               
            }            
        }
        
        public function getNotifications($userId, $limit = 5){
            if (is_numeric($userId) && is_int($limit)) {
                $dbData = DbHandler::get_instance()->ReturnQuery("SELECT user_notifications.id, user_notifications.datetime, "
