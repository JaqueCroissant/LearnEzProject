<?php
    class RightsHandler extends Handler
    {
        ...
        {
            if($this->user_exists()) {
                $userRights = DbHandler::get_instance()->ReturnQuery("SELECT rights.id, rights.prefix, rights.sort_order, translation_rights.title
        ...
        public function update_type_rights($user_type, $rights_array)
        {
            if(is_int($user_type) && $user_type < 5 && $user_type > 0)
            {
                ...
                $rights = DbHandler::get_instance()->ReturnQuery("SELECT * FROM rights");
