...
class SchoolHandler extends Handler {
    ...
    public function delete_school_by_id ($id) {
        try {
            ...
            if (DbHandler::getInstance()->Query($query, $id)) {
                return true;
            } else {
    ...
    public function get_school_by_id ($id) {
        try {
            ...
            $this->school = reset(DbHandler::getInstance()->ReturnQuery($query, $id));
            return true;
        } catch (Exception $exc) {
            $this->error = ErrorHandler::ReturnError($exc->getMessage());
            return false;
    ...
    public function update_school_by_id ($id, $name, $phone, $address, $email, $school_type_id, $max_students, $subscription_end) {
        try {
            ...
            if (DbHandler::getInstance()->Query($query, $name, $phone, $address, $email, $school_type_id, $max_students, $subscription_end, $id)) {
                return true;
            } else {
    ...
    private function verify_school_exists($id) {
        ...
        $count = DbHandler::getInstance()->CountQuery("SELECT * FROM school WHERE id = :id", $id);
        if (!($count == 1)) {
    ...
    private function verify_school_type($school_type_id) {
        ...
        $count = DbHandler::getInstance()->CountQuery("SELECT * FROM school_type WHERE id = :id", $school_type_id);
        if (!($count == 1)) {
            throw new Exception ("WRONG_SCHOOL_TYPE_ID");
        }
    }
