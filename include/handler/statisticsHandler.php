    ...
    public function get_top_students($limit = 5, $school = null, $class = null) {
        try {
            ...
            if ($has_school && !$has_class) {
                ...
                $data = DbHandler::get_instance()->return_query("SELECT users.id, users.username, users.firstname, users.surname, users.points, users.image_id, image.filename as profile_image, school.name, GROUP_CONCAT(class.title SEPARATOR ', ') AS classes FROM users LEFT JOIN image ON image.id = users.image_id INNER JOIN school on users.school_id = school.id INNER JOIN user_class ON users.id = user_class.users_id INNER JOIN class ON class.id = user_class.class_id WHERE users.user_type_id = 4 GROUP BY users.id, users.username, users.firstname, users.surname, users.points ORDER BY users.points DESC LIMIT " . $limit);
            } else if (!$has_school && $has_class) {
